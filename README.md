...
([synthetic.py](src/latemile/synthetic.py)): hub congestion, destination weather,
...
[olist.py](src/latemile/olist.py) is intentionally the template to copy when you write
...
1. Produce a DataFrame with the canonical columns in
   [schema.py](src/latemile/schema.py). Only three are truly required to be meaningful:
...
3. Run `latemile all` and read `artifacts/reports/`. Then score new shipments with
   `latemile score --input tomorrow.csv`.
...
src/latemile/
  ...
  cli.py         latemile generate | all | score
